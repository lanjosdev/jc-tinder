...
export default function Profile() {
    ...
    const [genderSelect, setGenderSelect] = useState(null); //=// pegar dados de id no profileDetails ou get id user
    const [genderOptionalSelect, setGenderOptionalSelect] = useState(null); //=// pegar dados de id no profileDetails ou get id user
    const [sexualitySelect, setSexualitySelect] = useState(null); //id=// pegar dados de id no profileDetails ou get id user
        ...
        async function getAllGenders() {
            ...
            try {
                ...
                if(response.success) {
                    ...
                    //=// Paliativo enquanto não tem os id de gender no profileDetails
                    response.data.forEach((item)=> {
                        if(item.name == profileDetails.gender) {
        ...
        async function getAllGendersOptionals() {
            ...
            try {
                ...
                if(response.success) {
                    ...
                    //=// Paliativo enquanto não tem os id de seb_gender no profileDetails
                    if(profileDetails.sub_gender) {
                        response.data.forEach((item)=> {
                            if(item.name == profileDetails.sub_gender) {
                                setGenderOptionalSelect(item);
                            }
                        });
                    }
        ...
        async function getAllSexualities() {
            ...
            try {
                ...
                if(response.success) {
                    ...
                    //=// Paliativo enquanto não tem o id de sexuality no profileDetails
                    response.data.forEach((item)=> {
                        if(item.name == profileDetails.sexuality) {
                            setSexualitySelect(item);
                        }
                    });
                    setError(false);
                }
    ...
    function handleChangeGender(selectGender) {
        ...
        //=// preciso do objeto gender
        // if(selectGender.name == profileDetails.gender) {
            ...
            <main className='PageContent FormsContent ProfileContent grid'>
                ...
                <div className="content_main">
                    ...
                    <form className="form" onSubmit={handleSubmitProfileUpdate} autoComplete="off">
                        ...
                        {((name !== profileDetails.name || dateBirth !== profileDetails.birth_data || phone !== profileDetails.phone || genderSelect?.name !== profileDetails.gender || genderOptionalSelect?.name !== profileDetails.sub_gender || sexualitySelect?.name !== profileDetails.sexuality || aboutMe !== '') && (phone.length == 11 && name !== '')) && 
                        (
